...
def Plot(city, R, dist, pause_interval=0.2):
    ...
    plt.pause(pause_interval)  # Pause for the interval


if __name__ == '__main__':
    nct = 100  # Number of cities to visit
    ...
    for t in range(maxTsteps):  # For each time step;
        ...
        for i in range(maxSteps):  # At each temperature;
            ...
            if de < 0:
                # the delta-E is either negative (better path found) or positive but allowed with a probability determined by T
